            ...
            if resp.status_code == 200:
                ...
                for ann in anns:
                    ...
                    if str(year) in title:
                        if report_type == "annual":
                            if "年度报告" in title and "摘要" not in title and "H股" not in title:
...
def main():
    ...
    for i, row in enumerate(rows):
        ...
        year_str = str(row['year'])  # 确保是字符串
        report_type = row['report_type']
        
        print(f"\r处理中: {i+1}/{len(rows)} - {stock_code} {year_str} {report_type}", end='')
